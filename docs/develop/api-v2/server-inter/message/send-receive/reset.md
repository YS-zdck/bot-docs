...
用于撤回当前群 `group_openid` 中发送不超过 **2 分钟** 的消息。

- 群管理员机器人可以撤回机器人自己的消息，也可以撤回普通群成员的消息。
- 普通成员机器人只能撤回机器人自己发送的消息。
- 普通群成员消息的 `message_id` 从 `GROUP_AT_MESSAGE_CREATE` 或 `GROUP_MESSAGE_CREATE` 事件的 `d.id` 获取。
- 机器人自己发送的消息 ID 从消息发送接口响应获取。
...
| --- | --- | --- |
| 40061001 | 请求参数无效 | 请检查请求参数格式 |
| 40062003 | 无操作权限 | 检查机器人是否为群管理员，或被撤回消息的发送者是否为普通群成员 |
| 40064004 | 已超出消息撤回时限 | 消息发送超过 2 分钟后不可撤回 |
