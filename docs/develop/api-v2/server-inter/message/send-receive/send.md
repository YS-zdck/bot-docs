...
- **2026/06/22**  
  群场景主动推送消息能力全量开放，群主开启设置后，机器人可对群下发主动消息。  
  频控规则：  
  · 机器人维度：企业/个人认证 60 qpm，未认证 30 qpm  
  · 单群维度：20 qpm
...
- **2026/01/10**  
  单聊场景新增互动召回消息能力：用户主动对话后，机器人在未来30天内可分4个周期（当天、1-3天、3-7天、7-30天）各下发1条召回消息。  
  单聊发消息接口新增 `is_wakeup` 字段。被动消息次数由60分钟5次调整为60分钟4次。

- **2025/04/21 起**  
  主动推送能力不再提供，接口调用将返回错误。
